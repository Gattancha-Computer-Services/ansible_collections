...
def system_sso_fortigate_cloud_admin(data, fos):
    ...
    if state == "present" or state is True:
        ...
        return fos.delete(
            "system", "sso-fortigate-cloud-admin", mkey=filtered_data["name"], vdom=vdom
        )
    else:
...
def fortios_system(data, fos):
    ...
    fos.do_member_operation("system", "sso-fortigate-cloud-admin")
    if data["system_sso_fortigate_cloud_admin"]:
        resp = system_sso_fortigate_cloud_admin(data, fos)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "accprofile": {"revisions": {"v7.4.0": True, "v7.2.4": True}, "type": "string"},
        "vdom": {
            "type": "list",
            "elements": "dict",
            "children": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sso_fortigate_cloud_admin"]["options"][
            attribute_name
