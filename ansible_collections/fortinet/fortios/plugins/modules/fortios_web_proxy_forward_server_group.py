...
def web_proxy_forward_server_group(data, fos, check_mode=False):
    ...
    if state == "present" or state is True:
        ...
        return fos.delete(
            "web-proxy", "forward-server-group", mkey=filtered_data["name"], vdom=vdom
        )
    else:
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    fos.do_member_operation("web-proxy", "forward-server-group")
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_forward_server_group"]["options"][
            attribute_name
